    ...
    <div className="sticky top-0 z-30 glass border-b border-border/60 pl-16 pr-4 py-3 lg:px-6 lg:py-3">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {/* Search */}
          <motion.div animate={{ width: searchOpen ? 240 : 36 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}
            className="relative overflow-hidden">
            <button onClick={() => setSearchOpen(!searchOpen)}
              className={cn("absolute left-0 top-0 h-9 w-9 flex items-center justify-center rounded-lg transition-colors z-10 cursor-pointer",
          ...
          <div className="relative">
            <button className="h-9 w-9 flex items-center justify-center rounded-lg text-text-muted hover:text-text-primary hover:bg-raised transition-colors cursor-pointer">
